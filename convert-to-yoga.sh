...
# Convert Food Theme to EatMe Theme
# Run this script from the project root

echo "Converting project from Food theme to EatMe theme..."

# Find all TypeScript/JavaScript files
find ./client -type f \( -name "*.tsx" -o -name "*.ts" -o -name "*.jsx" -o -name "*.js" \) -exec sed -i '' \
  -e 's/TheCookStudio/TheEatMeStudio/g' \
  ...
  -e 's/Food/EatMe/g' \
  -e 's/food/EatMe/g' \
  -e 's/Culinary/EatMe/g' \
  -e 's/culinary/EatMe/g' \
  -e 's/EatMe/EatMeFlow/g' \
  -e 's/bg-food-cream/bg-EatMe-cream/g' \
  -e 's/bg-food-brown/bg-EatMe-purple/g' \
